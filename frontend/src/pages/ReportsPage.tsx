  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500">Automated Markdown and PDF intelligence reports generated after each scraping & LLM analysis cycle.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Reports Archive List */}
        <div className="space-y-3">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">Generated Reports ({reports.length})</h3>
              ...
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all ${
                  selectedReport?.id === rep.id 
                    ? 'bg-cyan-500/10 border-cyan-500/30 text-slate-800 shadow-sm' 
                    : 'bg-darkCard border-darkBorder text-slate-600 hover:border-slate-400'
                }`}
              >
                <div className="text-xs font-bold text-slate-800 flex items-center justify-between mb-1">
                  <span>{rep.title}</span>
                  <span className="text-[10px] bg-cyan-500/10 text-cyan-600 px-2 py-0.5 rounded font-semibold">{rep.period}</span>
                </div>
        ...
        <div className="lg:col-span-2 glass-card p-6 rounded-2xl border border-darkBorder space-y-6 bg-darkCard">
          {selectedReport ? (
            <>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-darkBorder">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Generated on {new Date(selectedReport.created_at).toLocaleString()}</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <a
                    ...
                    download
                    className="flex items-center gap-2 px-3 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-cyan-500/20"
                  >
              ...
              <div className="p-4 rounded-xl bg-darkBg border border-darkBorder space-y-2">
                <div className="text-xs font-bold text-cyan-600 uppercase tracking-wider flex items-center gap-2">
              ...
              <div>
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Monitored Channels Covered</div>
                <div className="flex flex-wrap gap-2">
                  {selectedReport.channels_analyzed.map((ch, idx) => (
                    <span key={idx} className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-mono border border-slate-200">
                      @{ch}
                    </span>
                  ))}
                </div>
