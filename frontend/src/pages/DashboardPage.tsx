    ...
    <div className="space-y-5 w-full relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">Real-time Telegram Channel Monitoring</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1 */}
        <div className="glass-card p-5 rounded-xl flex items-center justify-between border border-darkBorder bg-darkCard/40">
          <div className="space-y-1">
            <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Channels Monitored</div>
            <div className="text-2xl font-bold text-slate-100">{monitoredList.length}</div>
            <div className="text-[10px] text-slate-500 font-medium">Total Channels: {channels.length}</div>
          </div>
          <div className="w-11 h-11 rounded-lg bg-cyan-500/10 text-cyan-400 flex items-center justify-center border border-cyan-500/20">
        ...
        <div className="glass-card p-5 rounded-xl flex items-center justify-between border border-darkBorder bg-darkCard/40">
          <div className="space-y-1">
            <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Total Messages</div>
            <div className="text-2xl font-bold text-slate-100">{messages.length}</div>
            <div className="text-[10px] text-slate-500 font-medium">Messages Scraped</div>
          </div>
        ...
        <div className="glass-card p-5 rounded-xl flex items-center justify-between border border-darkBorder bg-darkCard/40">
          <div className="space-y-1">
            <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Active Jobs</div>
            <div className="text-2xl font-bold text-slate-100">{status.is_scraping ? '1' : '0'}</div>
            <div className="text-[10px] text-slate-500 font-medium">{status.is_scraping ? 'Running Now' : 'Standby'}</div>
          </div>
        ...
        <div className="glass-card p-5 rounded-xl flex items-center justify-between border border-darkBorder bg-darkCard/40">
          <div className="space-y-1">
            <div className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Reports Generated</div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-darkCard/20 p-3 rounded-xl border border-darkBorder">
        {/* Search */}
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-darkBg text-xs text-white pl-10 pr-4 py-2.5 rounded-lg border border-darkBorder focus:outline-none focus:border-slate-500"
          />
        </div>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto justify-end">
            ...
            <div className="flex bg-darkBg rounded-lg p-0.5 border border-darkBorder">
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all font-bold ${
                  filterType === 'ALL' ? 'bg-blue-600/10 text-blue-400 border border-blue-500/20' : 'hover:text-white'
                }`}
              >
                All
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all font-bold ${
                  filterType === 'GROUPS' ? 'bg-blue-600/10 text-blue-400 border border-blue-500/20' : 'hover:text-white'
                }`}
              >
                Groups
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all font-bold ${
                  filterType === 'CHANNELS' ? 'bg-blue-600/10 text-blue-400 border border-blue-500/20' : 'hover:text-white'
                }`}
              >
                Channels
              </button>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-400">
            ...
            <select
              ...
              className="bg-darkBg text-xs text-slate-200 px-3 py-1.5 rounded-lg border border-darkBorder focus:outline-none cursor-pointer font-bold"
            >
      ...
      {/* CHANNELS DATA TABLE */}
      <div className="glass-card rounded-xl border border-darkBorder overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-darkCard/80 border-b border-darkBorder text-[10px] font-bold text-slate-400 uppercase tracking-wider">
            ...
            <tbody className="divide-y divide-darkBorder/60 text-xs text-slate-300">
                ...
                <tr key={ch.id} className="hover:bg-darkCard/40 transition-colors group">
                  ...
                  <td className="py-4 px-4 text-center">
                    <button onClick={() => handleToggle(ch.id)} className="text-blue-500 hover:text-blue-400">
                      {ch.is_monitored ? <CheckSquare className="w-4 h-4 text-blue-500" /> : <Square className="w-4 h-4 text-slate-600" />}
                    </button>
                  </td>

                  {/* Avatar Icon */}
                  <td className="py-4 px-3 text-center" onClick={() => openChannelInNewTab(ch.id)}>
                    <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-slate-700 to-slate-800 border border-white/10 text-slate-200 flex items-center justify-center mx-auto cursor-pointer font-bold text-[10px]">
                  ...
                  <td className="py-4 px-4 cursor-pointer" onClick={() => openChannelInNewTab(ch.id)}>
                    <div className="font-bold text-slate-100 text-sm group-hover:text-blue-400 transition-colors">{ch.title}</div>
                    <div className="text-[10px] text-slate-500 font-mono">ID: {ch.id}</div>
                  </td>

                  {/* Username Link */}
                  <td className="py-4 px-4 font-mono text-blue-400 font-bold hover:underline cursor-pointer" onClick={() => openChannelInNewTab(ch.id)}>
                    {ch.username}
                  </td>
                  ...
                  <td className="py-4 px-4 text-center">
                    {ch.status === 'scraping' ? (
                      <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                        🟡 Scraping
                        <span className="block text-[8px] opacity-75 font-normal">Auto every 30 min</span>
                      </span>
                    ) : ch.is_auto_monitoring ? (
                      <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                        🟢 Monitoring
                        <span className="block text-[8px] opacity-75 font-normal">Auto every {ch.monitoring_interval_value} min</span>
                      </span>
                    ) : (
                      <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-slate-800 text-slate-400 border border-slate-700">
                        ⚪ Idle
                  ...
                  <td className="py-4 px-4 text-right pr-6 space-x-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 border border-blue-500/20 font-bold text-[11px] transition-all"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 p-1.5 rounded-lg text-slate-500 hover:text-slate-200"
                    >
      ...
      <div className="glass-card p-5 rounded-xl border border-darkBorder flex flex-col h-[280px]">
        ...
        <div className="flex-1 bg-darkBg/90 rounded-lg p-4 font-mono text-[11px] text-emerald-400 overflow-y-auto space-y-1 border border-white/5 shadow-inner">
